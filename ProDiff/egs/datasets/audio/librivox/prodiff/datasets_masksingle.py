...
class LabelSpeechDatasetMaskSingleFairseq(BaseDataset):
    def __init__(self, phone_encoder, prefix, shuffle=False):
        ...
        if not self.is_infer:
            ...
            self.feats, self.feat_lengs, self.feat_ranks, self.feat_offsets = [], [], [], []
        ...
        if not self.is_infer:
            ...
            filtered_indices = list(range(len(self.labels)))
        ...
        if not self.is_infer:
            ...
            self.sizes = [int(wl / self.sr * self.feat_rate) for wl in self.wav_lengs]
            # print(self.sizes)
    ...
    def mask_phone(self, phone):
        ...
        return phone, startid, endid

    
    def __getitem__(self, index):
        hparams = self.hparams
        wav_path = self.wav_paths[index]
        spk, book, ix = re.match('.*[_/](\d+)-(\d+)-(\d+)[_.].*', wav_path).groups()
        item_name = f'{spk}-{book}-{ix}'
        # item_name = Path(wav_path).stem.replace('_', '-') 
        spkid, *_ = item_name.split('-')
        ...
        np.random.seed(hparams['seed']+index)
        phone, si, ei = self.mask_phone(phone)
        # print(f'setseed {hparams["seed"]+index} item {item_name} start {si} end {ei}')


        label_leng = int(len(phone) * self.label_resample_rate)
        spec = mel = torch.FloatTensor(feat) if feat is not None else torch.zeros(label_leng, hparams['audio_num_mel_bins']).float()
