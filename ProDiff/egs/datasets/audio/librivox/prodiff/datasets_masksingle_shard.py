...
class LabelSpeechDatasetMaskSingleFairseqShard(BaseDataset):
    def __init__(self, phone_encoder, prefix, shuffle=False):
        ...
        self.gen_nshard = hparams.get('gen_nshard', 1)
        self.gen_rank = hparams.get('gen_rank', 0)
        ...
        tot = len(self.feat_lengs)
        start, end = get_shard_range(tot, self.gen_nshard, self.gen_rank)
        filtered_indices = list(range(len(self.feat_lengs)))[start:end]
